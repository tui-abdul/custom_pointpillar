...
import os
from utils import bbox3d2corners_custom, bbox3d2corners
...
def vis_pc(pc, bboxes=None, labels=None,scores=None):
    '''
    pc: ply or np.ndarray (N, 4)
    bboxes: np.ndarray, (n, 7) or (n, 8, 3)
    labels: (n, )
    '''
    #bboxes = bboxes[np.where(scores > 0.1)[0]] 
    ...
    if len(bboxes.shape) == 2:
        ...
        bboxes = bbox3d2corners_custom(bboxes)
