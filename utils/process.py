...
def bbox3d2bevcorners(bboxes):
    ...
    return: shape=(n, 4, 2)
    '''
    #print("bbox3d2bevcorners", bboxes.shape)
...
def bbox3d2corners_custom(bboxes):
    '''
    bboxes: shape=(n, 7)
    return: shape=(n, 8, 3)
           ^ z   x            6 ------ 5
    ...
    '''
    centers, dims, angles = bboxes[:, :3], bboxes[:, 3:6], bboxes[:, 6]
    #dims, centers, angles = bboxes[:, :3], bboxes[:, 3:6], bboxes[:, 6]
    #print(dims.shape)
    #print("bboxes",bboxes )
    #print("centers, dims, angles",centers,dims,angles)   
    #dims = dims[:,(2,0,1) ] 
    # 1.generate bbox corner coordinates, clockwise from minimal point
    ...
    # 2. rotate around z axis
    rot_sin, rot_cos = np.sin(angles), np.cos(angles)
    # in fact, -angle is not bcoz we are only using yaw angle from lidar data
    rot_mat = np.array([[rot_cos, -rot_sin, np.zeros_like(rot_cos)],
                        [rot_sin, rot_cos, np.zeros_like(rot_cos)],
                        [np.zeros_like(rot_cos), np.zeros_like(rot_cos), np.ones_like(rot_cos)]], 
                        dtype=np.float32) # (3, 3, n)
...
def iou3d_custom(bboxes1, bboxes2,score):
    """
    Computes the 3D IoU between two sets of 3D bounding boxes.
    
    Args:
        bboxes1: (n, 7) Tensor containing (x, y, z, w, l, h, theta)
        bboxes2: (m, 7) Tensor containing (x, y, z, w, l, h, theta)
    
    Returns:
        iou: (n, m) Tensor with IoU values.
    """
    # Height overlap
    b1_z_min, b1_z_max = bboxes1[:, 2], bboxes1[:, 2] + bboxes1[:, 5]
    b2_z_min, b2_z_max = bboxes2[:, 2], bboxes2[:, 2] + bboxes2[:, 5]
    
    z_min = torch.maximum(b1_z_min[:, None], b2_z_min[None, :])
    z_max = torch.minimum(b1_z_max[:, None], b2_z_max[None, :])
    height_overlap = torch.clamp(z_max - z_min, min=0)
    
    # BEV (Bird's Eye View) overlap
    b1_xy_min = bboxes1[:, :2] - bboxes1[:, 3:5] / 2
    b1_xy_max = bboxes1[:, :2] + bboxes1[:, 3:5] / 2
    b2_xy_min = bboxes2[:, :2] - bboxes2[:, 3:5] / 2
    b2_xy_max = bboxes2[:, :2] + bboxes2[:, 3:5] / 2
    
    b1_bev = torch.cat([b1_xy_min, b1_xy_max, bboxes1[:, 6:]], dim=-1)
    b2_bev = torch.cat([b2_xy_min, b2_xy_max, bboxes2[:, 6:]], dim=-1)
    bev_overlap = boxes_overlap_bev(b1_bev, b2_bev)  # (n, m)
    
    # Overlapping volume
    overlap_volume = height_overlap * bev_overlap
    volume1 = (bboxes1[:, 3] * bboxes1[:, 4] * bboxes1[:, 5])[:, None]
    volume2 = (bboxes2[:, 3] * bboxes2[:, 4] * bboxes2[:, 5])[None, :]
    total_volume = volume1 + volume2
    
    # IoU calculation
    iou = overlap_volume / (total_volume - overlap_volume + 1e-8)
    #print("iou",iou)
    return iou
...
def keep_bbox_from_lidar_range(result, pcd_limit_range):
    ...
    #lidar_bboxes = result['lidar_bboxes'], result['labels'], result['scores']
...
def points_in_bboxes_v3(points, dimensions, location, rotation_y, name):
    ...
    #bboxes_lidar = bbox_camera2lidar(bboxes_camera, tr_velo_to_cam, r0_rect)
    bboxes_corners = bbox3d2corners_custom(bboxes_lidar)
    group_rectangle_vertexs_v = group_rectangle_vertexs(bboxes_corners)
    frustum_surfaces = group_plane_equation(group_rectangle_vertexs_v)
    indices = points_in_bboxes(points[:, :3], frustum_surfaces) # (N, n), N is points num, n is bboxes number
    return indices, n_total_bbox, n_valid_bbox, bboxes_lidar, name
...
def get_points_num_in_bbox_custom(points, dimensions, location, rotation_y, name):
    '''
    points: shape=(N, 4) 
    tr_velo_to_cam: shape=(4, 4)
    r0_rect: shape=(4, 4)
