...
def create_data_info_pkl(data_root, data_type, prefix, label=True, db=False):
    ...
    ids_file = os.path.join(CUR, 'dataset', 'ImageSets','old', f'{data_type}.txt')
    ...
    for id in tqdm(ids):
        ...
        write_points(lidar_points, saved_reduced_points_name)
        ...
        if label:
            ...
            #print("anno dict",annotation_dict)
            annotation_dict['num_points_in_gt'] = get_points_num_in_bbox_custom(
                points=lidar_points,
                #r0_rect=calib_dict['R0_rect'], 
                #tr_velo_to_cam=calib_dict['Tr_velo_to_cam'],
