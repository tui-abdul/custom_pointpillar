...
def main(args):
    ...
    pcd_limit_range = np.array([0, -39.68, -5, 69.12, 39.68, 1], dtype=np.float32)
    ...
    print("scoresssssssssssssss",lidar_bboxes.shape, labels.shape, scores)
    vis_pc(pc, bboxes=lidar_bboxes, labels=labels,scores=scores)
    ...
    if calib_info is not None and gt_label is not None:
        ...
        vis_pc(pc, pred_gt_lidar_bboxes, labels=pred_gt_labels,scores=scores)
