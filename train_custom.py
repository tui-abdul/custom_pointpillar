...
def main(args):
    ...
    for epoch in range(args.max_epoch):
        ...
        if (epoch + 1) % args.ckpt_freq_epoch == 0:
            torch.save(pointpillars.state_dict(), os.path.join(saved_ckpt_path, f'epoch_shifted_360{epoch+1}.pth'))
        if epoch  == 29:    
            print("weights + model archi saved")
            torch.save(pointpillars, os.path.join(saved_ckpt_path, f'epoch_kitti_shifted_360_archi{epoch+1}.pth'))
        if epoch % 2 == 0:
            continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_freq_epoch', type=int, default=30)
