...
from dataset import Kitti,KittiIntersection, get_dataloader
...
def main(args):
    setup_seed()
    train_dataset = KittiIntersection(data_root=args.data_root,
                          split='train')
    val_dataset = KittiIntersection(data_root=args.data_root,
                        split='val')
    ...
    if not args.no_cuda:
        ...
        #pointpillars = torch.load("pillar_logs/checkpoints/epoch_kitti_shifted_360_archi30.pth")
    else:
    ...
    for epoch in range(args.max_epoch):
        ...
        for i, data_dict in enumerate(tqdm(train_dataloader)):
            ...
            #print("batched_pts",batched_pts[0].shape)
            #print("batched_gt_bboxes",batched_gt_bboxes[0])
            #print("batched_labels",batched_labels[0])
        ...
        if (epoch + 1) % args.ckpt_freq_epoch == 0:
            #torch.save(pointpillars.state_dict(), os.path.join(saved_ckpt_path, f'epoch_fine{epoch+1}.pth'))
            pass
        if epoch  == 29:
        ...
        print(f"Loss: {loss.item():.4f}, Learning Rate: {optimizer.param_groups[0]['lr']:.6f}")
        print("-" * 50)
